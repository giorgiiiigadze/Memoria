...
## Use native SwiftUI/UIKit components everywhere — hard requirement

The entire point of this rebuild is going 100% native. Wherever iOS/SwiftUI already ships a control for something, use it as-is with system styling — do not recreate a custom-drawn version of a control the OS gives you for free. This isn't a style preference, it's the reason to do this rebuild instead of just polishing the RN app. Concretely:

| UI need | Use this native API | Notes |
|---|---|---|
| Floating chrome/panels/buttons over photos (Drop Detail header, Story header, invite card) | **Liquid Glass**: `.glassEffect(_:in:)` with `Glass.regular.tint(_:).interactive()`; group multiple glass elements in one `GlassEffectContainer` with per-element `.glassEffectID` so they share refraction and can morph (e.g. back + `•••` buttons in the same header row) | Requires the iOS 26 SDK; gate with `if #available(iOS 26, *)` and fall back to `.background(.ultraThinMaterial, in: shape)` on older versions |
| Tab bar | `TabView` | Automatically glass-styled on iOS 26 — don't hand-roll blur/tint to match `blurEffect="systemUltraThinMaterialDark"` from the RN app |
| Nav bar, toolbar buttons/badges, modal close button | `NavigationStack` + `.toolbar { ToolbarItem { Button(...) } }`, `.badge()` | Applies to the bell+badge and `+` buttons on Home, the gear icon on Profile, and the `xmark` close button on Settings — these are already plain native toolbar buttons in the RN source (`Stack.Toolbar.Button`), not custom views |
| Segmented tab switcher (Calendar "All Drops" / "My Drops") | `Picker("", selection:) { ... }.pickerStyle(.segmented)` | The RN source already renders this via a native `SegmentedControl` bridge — it's meant to look and behave exactly like the system control |
| Forms with rows/sections (Create Drop details screen, Settings screen) | `Form { Section { ... } }` with native `TextField`, `DatePicker`, `Toggle`, `Button` | The RN source **already builds these two screens out of `@expo/ui/swift-ui`'s native Form/Section/TextField/DatePicker/Button** — i.e. they're already real native SwiftUI under an Expo bridge. Port them 1:1 to a genuine SwiftUI `Form`; don't reinvent a custom-styled list for either screen |
| Destructive/action menus (Drop Detail `•••` → "Delete Drop"; Story viewer photo menu) | `.confirmationDialog(...)` | Replaces the RN app's `ActionSheetIOS` |
| Long-press actions on grid items (pin/delete a photo; pin a drop from a mini-card grid) | `.contextMenu { Button(...) }` | Matches the RN app's long-press-to-act pattern on grid cells |
| Pull-to-refresh (Home feed, Friends, Calendar) | `.refreshable { await ... }` | Replaces the RN app's custom animated `RefreshControl` + grid-reveal effect — a plain native refresh spinner is correct here |
| Alerts / destructive confirmations (delete drop, delete photo, generic errors) | `.alert(...)` | |
| Photo library picking (avatar, drop cover fallback, contacts screen) | `PhotosPicker` (PhotosUI) | |
| In-app camera capture (drop cover photo, photo upload to a drop) | `UIImagePickerController` (camera source) bridged via `UIViewControllerRepresentable` | SwiftUI has no first-party pure-SwiftUI camera capture view; bridging the system camera controller is the native-idiomatic choice, matching what `expo-image-picker`'s camera launcher already does under the hood |
| Date selection (drop "Opens" date) | `DatePicker` | |
| Contacts access | `Contacts` framework (`CNContactStore`) | |
| Photo/mini-card grids | `LazyVGrid` / `LazyHGrid` | |

Only reach for a fully custom-drawn view where there is no system equivalent at all — e.g. the Story viewer's full-screen photo pager with drag-to-dismiss, and the bespoke visual design of drop cards / mini drop cards. Everything else in the table above should be the real system control, not a look-alike.
...
**Glass surfaces**: panel tint ≈ `Color.white.opacity(0.08)` on dark backgrounds; floating chrome over photos uses a `Color.black.opacity(0.35)` fallback with a `Color.white.opacity(0.2)` hairline border. See "Use native SwiftUI/UIKit components everywhere" above for the actual `.glassEffect()`/`GlassEffectContainer` API and which specific UI elements use it.
...
- `.refreshable { }` re-fetches drops (native pull-to-refresh).
...
Two-step flow:
1. **Details form**, built as a native `Form { Section { ... } }` (mirroring the RN source, which already renders this screen with `@expo/ui/swift-ui`'s native Form): title `TextField`, an "Opens" `DatePicker` (must be today or later; default tomorrow), and a scrollable friend-invite list (checkable rows, each friend toggled in/out of `invitedIds`). A checkmark toolbar button (enabled once title + date are set) opens the system camera (`UIImagePickerController` bridge) to shoot a cover photo, then advances.
...
- Header overlay: back chevron button and, for the creator only, a `•••` button — both rendered with `.glassEffect()` (grouped in one `GlassEffectContainer`); the `•••` button opens a `.confirmationDialog` with "Delete Drop" (destructive, confirm-again alert; deletes the row, which cascades photos via FK).
- If the drop's `state` is `active` or `ready`, photos are **locked** — tapping one shows a `.alert` instead of opening it ("Photos will be revealed when this drop opens"). Once `open`/`expired`, tapping a photo pushes the full-screen Story viewer at that photo's index.
- A floating shutter button (visible only to eligible participants while the drop is `active`/`ready`) opens the camera (`UIImagePickerController` bridge); on capture, upload via `PhotosService`, `INSERT INTO photos`, and mark the uploader's `drop_participants` row `has_uploaded = true`, `status = accepted`.
- Long-press a photo (creator or uploader) to open a native `.contextMenu` with pin/unpin (`is_pinned`, re-sorts pinned-first) and delete (removes Storage object + row, with optimistic UI + rollback + alert on failure).
...
- Header overlay (glass, per the native-components mandate above): back/down chevron button, uploader avatar + name + relative "time ago", and a `•••` button opening a `.confirmationDialog` (pin toggle, "save" — currently a stub "coming soon" alert in the RN app, fine to leave as a stub or wire up to Photos library export).
...
- Not in search mode: an "Invite your friends" `.glassEffect()` card (opens the system share sheet), a "Suggested" section (top 3 contacts-on-Memoria not already friended/pending, from the Contacts-matching flow — same normalization logic as onboarding step 6) with a "see all" link to a dedicated Contacts screen, an incoming "Requests" section (Accept/Decline), and the full friends list (name + "friends since" relative date).
...
- Two horizontally-paged views ("All Drops" / "My Drops" — a native `Picker(.segmented)` header synced to page position), each grouping the same drops by month of `open_date` (drops with no `open_date` grouped last under "No date"), rendered as a `LazyVGrid` of mini drop cards per month.

## Feature: Profile

- Avatar, display name, `@username`, bio, and a stats row (total drops / collecting / ready / open counts).
- Pinned drops section (if any) + all-drops `LazyVGrid`, both as tappable mini cards; long-press opens a native `.contextMenu` to toggle `is_pinned` optimistically.
- Settings screen (pushed via a gear toolbar button), built as a native `Form { Section("Account") { ... } }` — mirroring the RN source, which already renders this screen with `@expo/ui/swift-ui`'s native Form: a single destructive "Sign Out" `Button`, and nothing else currently — keep it minimal, matching the source.
